...
class micro_targeting_recommender():
    ...
    def get_data(self):
        ...
        and grt_l3_cat_name in (select index_name from  push_analytics.xm_index a  inner join  
        push_analytics.xm_tb1 b
        on a.promo_id = b.promo_id where promo_date = "{0}" and sub_category='L3')
        """.format(self.event_date)
        #print(query)
        test_df=spark.sql(query)\
        .select(['grt_l3_cat_name','event_date','consumer_id','deal_uuid','deal_views','grt_l3_purchaser_14d','purchaser_14d','unique_deal_views'])
    ...
    def pre_process_data(self,raw_df):
        df=raw_df\
        .select(['grt_l3_cat_name','event_date','consumer_id','deal_uuid','deal_views','grt_l3_purchaser_14d','purchaser_14d','unique_deal_views'])\
        .groupby(['consumer_id','grt_l3_cat_name','event_date'])\
        .agg(F.sum(col('deal_views')).alias('udv')
            ,F.sum(col('unique_deal_views')).alias('dv')
            ,F.countDistinct(col('deal_uuid')).alias('ud')
            ,F.max(col('grt_l3_purchaser_14d')).alias('grt_l3_purchaser_14d')
            ,F.max(col('purchaser_14d')).alias('purchaesr_14d'))

        df.createOrReplaceTempView("df") 

        df_processed=spark.sql(
            """SELECT *,sum(dv) OVER ( PARTITION BY consumer_id,grt_l3_cat_name ORDER BY datediff(event_date,'2007-01-01') 
            RANGE BETWEEN  29  PRECEDING AND CURRENT ROW) AS dvs_p30days
            ,sum(udv) OVER ( PARTITION BY consumer_id,grt_l3_cat_name ORDER BY datediff(event_date,'2007-01-01')
            RANGE BETWEEN  29 PRECEDING AND CURRENT ROW) AS udvs_p30days 
            ,sum(ud) OVER ( PARTITION BY consumer_id,grt_l3_cat_name ORDER BY datediff(event_date,'2007-01-01') 
            RANGE BETWEEN 29  PRECEDING AND CURRENT ROW) AS ud_p30days 
            ,row_number() OVER (PARTITION BY consumer_id,grt_l3_cat_name ORDER BY datediff(event_date,'2007-01-01') desc) AS recent_event_date
             FROM df
    ...
    def baseline_model(self,df_processed):
        ...
        .filter(col('grt_l3_purchaser_14d')==0)
    ...
    def create_ouptput(self,df_micro):
        ...
        spark.sql("""drop table if exists push_analytics.promo_micro_v2_l3 purge """)
        spark.sql("""create table push_analytics.promo_micro_v2_l3 stored as orc 
        ...
        ,if(
           conv(
              substr(
                  md5(concat(a.consumer_id, '-','2020-microv1')),
                  1, 6),
...
micro_baseline_l3 = micro_targeting_recommender(event_date=event_date)
print('pass the first line')
raw_df_l3 = micro_baseline_l3.get_data()
print('pass the second line')
df_processed_l3 = micro_baseline_l3.pre_process_data(raw_df_l3)
print('pass the third line')
df_micro_l3 = micro_baseline_l3.baseline_model(df_processed_l3)
print('pass the fourth line')
micro_baseline_l3.create_ouptput(df_micro_l3) 
print('done')
spark.stop()
